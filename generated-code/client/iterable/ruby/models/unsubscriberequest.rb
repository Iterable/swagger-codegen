class UnsubscribeRequest
  ...
  def initialize(attributes = {})
    ...
    if attributes.to_s != ""
      ...
      if UnsubscribeRequest.attribute_map["list_id".to_sym] != nil
        name = "list_id".to_sym
        value = attributes["listId"]
        send("#{name}=", value) if self.respond_to?(name)
	      end
      if UnsubscribeRequest.attribute_map["subscribers".to_sym] != nil
        name = "subscribers".to_sym
        value = attributes["subscribers"]
        if value.is_a?(Array)
	        array = Array.new
	        value.each do |arrayValue|
	          array.push Unsubscriber.new(arrayValue)
	        end
  ...
  def to_body
    body = {}
    UnsubscribeRequest.attribute_map.each_pair do |key,value|
      v = self.send(key)
      if v.is_a?(Array)
        array = Array.new
